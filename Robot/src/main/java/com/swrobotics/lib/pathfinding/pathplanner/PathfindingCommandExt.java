...
public class PathfindingCommandExt extends Command {
    ...
    private final IntConsumer selectedGoalConsumer;
     ...
     *
     * @param environment the pathfinding environment to search in
     * @param targetPoses the poses to pathfind to, the rotation component is only relevant for
     ...
     * @param selectedGoalConsumer function to call once the goal has been selected
     ...
     */
    public PathfindingCommandExt(
            PathEnvironment environment,
            List<Pose2d> targetPoses,
            PathConstraints constraints,
            double goalEndVel,
            IntConsumer selectedGoalConsumer,
        ...
        this.goalVelocityMPS = goalEndVel;
        this.constraints = constraints;
        this.selectedGoalConsumer = selectedGoalConsumer;
    ...
    public void initialize() {
        ...
        if (!finish) {
            PathfinderExt pathfinder = AutoBuilderExt.getPathfinder();
    ...
    public void execute() {
        ...
        if (!skipUpdates && pathfinder.isNewPathAvailable()) {
            PathfinderExt.Path pathInfo = pathfinder.getCurrentPath(constraints, goalVelocityMPS);
            PathPlannerPath currentPath = pathInfo.path();
            if (selectedGoalConsumer != null)
                selectedGoalConsumer.accept(pathInfo.goalIndex());
    ...
    public void end(boolean interrupted) {
        ...
        if (!interrupted && goalVelocityMPS < 0.1) {
            output.accept(new ChassisSpeeds(), DriveFeedforwards.zeros(robotConfig.numModules));
        }
